...
def seller_lists_product(seller_loc, category, seller_price, condition="resell"):
    ...
    print(f" Seller Price:    Rs {seller_price}")
    print(f" Processing (origin): Rs {r_seller}")
    ...
    print(f"   {'->' if condition == 'resell' else ' '} Resell:    Rs {s_resell}")
    print(f"   {'->' if condition == 'refurbish' else ' '} Refurbish: Rs {s_refurbish}")
    print(f"   {'->' if condition == 'recycle' else ' '} Recycle:   Rs {s_recycle}")
    print(f" Selected ({condition.upper()}): Rs {best_info['S_i']}")
    print(f"{'='*60}")
...
def return_amazon_owned(warehouse_loc, category, original_purchase_price, customer_price, condition="resell"):
    ...
    print(f" Amazon Total Investment: Rs {original_purchase_price}")
    print(f" Customer Paid:     Rs {customer_price}")
    print(f"{'-'*60}")
    ...
    print(f"   {'->' if condition == 'refurbish' else ' '} Refurbish: Rs {s_refurbish}")
    print(f"   {'->' if condition == 'recycle' else ' '} Recycle:   Rs {s_recycle}")
    print(f" Selected ({condition.upper()}): Rs {s_i}")
    print(f"{'-'*60}")
    print(f" K for relist:                      Rs {round(k_relist, 2)}")
    print(f" Processing Fee (non-refundable):   Rs {r_warehouse} (warehouse: {warehouse_loc.upper()})")
    print(f" Refund to Customer:                Rs {refund_to_customer}")
    print(f"{'='*60}")

    if k_relist > 0:
        return {
            "decision": "RE-LIST AT SAME WAREHOUSE",
            "action": f"Product re-listed as {condition.upper()} at {warehouse_loc.upper()} for Rs {s_i}.",
        ...
        return {
            ...
            "action": (
                f"Re-listing unprofitable (K = Rs {round(k_relist, 2)}). "
...
def return_seller_owned(seller_loc, category, seller_price):
    ...
    print(f" Seller Price:       Rs {seller_price}")
    print(f"{'-'*60}")
    print(f" Customer Originally Paid:  Rs {customer_paid}")
    print(f"   (seller_price Rs {seller_price} + processing Rs {r_local} + listing Rs {listing_fee})")
    print(f"{'-'*60}")
    print(f" Amazon Deductions (non-refundable):")
    print(f"   Listing Fee:       Rs {listing_fee}")
    print(f"   Processing Fee:    Rs {r_local}")
    print(f"{'-'*60}")
    print(f" Refund to Buyer:     Rs {refund_to_buyer}")
    ...
    return {
        ...
        "action": (
            ...
            f"Amazon deducts Rs {listing_fee} (listing) + Rs {r_local} (processing) from refund."
        ),
...
if __name__ == "__main__":
    ...
    result2 = return_amazon_owned(
        ...
        condition="resell",  # Same as when listed
    )
    print("\n[DECISION]:")
