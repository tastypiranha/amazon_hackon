...
dataset_path = "/Users/highmonk/Amazon hackon/amazon_hackon/images_moniter/clothing "
...
def train_classifier():
    """Train a KNN classifier on the clothing dataset."""
    X = []
    y = []
    ...
    with open("classifier_clothing.pkl", "wb") as f:
        pickle.dump(clf, f)

    print("\nClassifier trained and saved to classifier_clothing.pkl")
    return clf


def classify_image(image_path, clf=None):
    """Classify a clothing image into recycle/refurbish/resell."""
    if clf is None:
        with open("classifier_clothing.pkl", "rb") as f:
            clf = pickle.load(f)
...
if __name__ == "__main__":
    ...
    print("Extracting features from all clothing images...")
    X = []
    y = []
    ...
    print("\n--- Test Results (Clothing) ---")
    print(f"Accuracy: {accuracy_score(y_test, y_pred):.2%}")
    print("\nClassification Report:")
    print(classification_report(y_test, y_pred))

    # Also train final model on ALL data and save it
    print("\n--- Training final model on all clothing data ---")
    clf_final = train_classifier()
    print("Done! Final model saved to classifier_clothing.pkl")
